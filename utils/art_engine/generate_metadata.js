...
const {
  format
} = require(`${FOLDERS.sourceDir}/artwork_config.js`);
...
const buildSetup = () => {
  if (fs.existsSync(FOLDERS.jsonDir)) {
    fs.rmSync(FOLDERS.jsonDir, { recursive: true });
  }
...
const writeMetaData = (_data) => {
  fs.writeFileSync(`${FOLDERS.buildDir}/_metadata.json`, _data);
};
