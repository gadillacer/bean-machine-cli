...
async function main() {
  ...
  if (totalFailedCount === 0) {
    ...
    console.log(`Re-Mint Process Is Needed, please run the utils/soulmint/remint.js script once ready.`);
  }
      ...
      let txnCheck = await fetch(`${jsonFile.mintData.transaction_external_url}`, options)
        ...
        .then(text => {
          ...
          if (text.toLowerCase().includes('search not found')) {
            ...
            // Write the contents of the minted json file into the failedMintsDir directory.
            fs.writeFileSync(`${FOLDERS.failedMintsDir}/${jsonFile.metaData.custom_fields.edition}.json`, JSON.stringify(jsonFile, null, 2));
            console.log(`${jsonFile.mintData.transaction_external_url} was minted, but the transaction has a failed status on the blockchain. Will remint ${FOLDERS.mintedDir}/${jsonFile.metaData.custom_fields.edition}.json`);
          }
